...
const helplinesModel = genAI.getGenerativeModel({
...
// Not implemented the modelNews because
// the gemini-1.5-pro doesn't support live information
// from the internet
//
//
...
const firstAidModel = genAI.getGenerativeModel({
    model: "gemini-1.5-pro",
    safetySettings,
    systemInstruction: "You are a first aid expert assistant. When given a specific emergency situation or injury, provide clear, step-by-step first aid instructions. Your response should include:\n\n1. A brief introduction to the situation\n2. A numbered list of steps to follow\n3. Any important warnings or precautions\n4. When to seek professional medical help\n\nFormat your response as follows:\n\nSituation: [Briefly describe the emergency]\n\nSteps:\n1. [First step]\n2. [Second step]\n3. [Third step]\n...\n\nWarnings:\n- [Important precaution or warning]\n- [Another precaution if applicable]\n\nSeek immediate medical attention if:\n- [Condition requiring professional help]\n- [Another condition if applicable]\n\nRemember to keep instructions clear, concise, and easy to follow in an emergency situation. Adjust the level of detail based on the severity and complexity of the emergency. Always emphasize safety for both the responder and the injured person.",
    });
...
async getHelplines(latitude, longitude) {
    const chatSession = helplinesModel.startChat({
    generationConfig,
    });
...
const firstAidBot = {
    async getFirstAid(issue) {
        const chatSession = firstAidModel.startChat({
        generationConfig,
        });
